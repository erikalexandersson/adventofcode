...
public class Day24Part2 {
	...
	public static void main(String args[]) throws URISyntaxException {
		System.out.println(
				findLongestStrongest(buildBridges(Paths.get(ClassLoader.getSystemResource("Day24Input.txt").toURI()))));
	}

	public static int findLongestStrongest(List<String> bridges) {
		int maxLength = bridges.stream().mapToInt(bridge -> bridge.split("--").length).max().getAsInt();

		Map<Integer, List<String>> bridgesWithLength = bridges.stream()
				.collect(Collectors.groupingBy(bridge -> bridge.split("--").length, TreeMap::new, Collectors.toList()));

		return bridgesWithLength.get(maxLength).stream().mapToInt(bridge -> getBridgeStrength(bridge)).max().getAsInt();
	}
